	...
	,login: function (btn) {
		...
		tmp.me.postAjax(tmp.me.getCallbackId('login'), tmp.data, {
			'onLoading': function () {
				...
				tmp.panel.addClassName('loading');
			}
			,'onSuccess': function(sender, param) {
				try {
					...
					tmp.panel.removeClassName('loading');
				}
			}
			,'onComplete': function(sender, param) {
				if(!tmp.panel.hasClassName('loading')) {
		...
		jQuery(tmp.me.jQueryFormSelector).bootstrapValidator({
	        ...
	        fields: {
	        		...
	        		validators: {
                        ...
                        remote: {
                            ...
                            data: {
                            	...
                            	'PRADO_CALLBACK_TARGET': tmp.me.getCallbackId('validUsername'),
                            	'PRADO_CALLBACK_PARAMETER': JSON.stringify({'query': jQuery(tmp.me.jQueryFormSelector).find('[name="email"]').val()})
                            }
                        }
	                }
	        	}
	        	,'org-name': {
	        		validators: {
	        			notEmpty: {
	        				message: 'Organization/Company is required'
	        			}
	        		}
	        	}
	        }
