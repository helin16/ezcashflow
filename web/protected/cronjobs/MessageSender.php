...
abstract class MessageSender
{
	public static function run()
	{
		...
		foreach	($messages as $message){
			...
			try {
				...
				EmailSenderAbstract::sendEmail($message->getTo(), $message->getSubject(), $message->getBody(), '', $message->getFrom(), 'EZBK.com.au', true, array());
				...
				$message->setSendType(Message::SENT_TYPE_FAILED)
					->save();
	...
	private static function _getAndMarkMessages()
	{
		...
		Message::updateByCriteria('transId = ?, sendType = ?', 'active = 1 and sendType = ?', array($randId, Message::SENT_TYPE_SENDING, Message::SENT_TYPE_NEW));
		return Message::getAllByCriteria('transId = ? and sendType = ?', array($randId, Message::SENT_TYPE_SENDING));
	}
}
